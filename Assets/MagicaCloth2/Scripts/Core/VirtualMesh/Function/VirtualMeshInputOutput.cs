...
namespace MagicaCloth2
{
    public partial class VirtualMesh
    {
        ...
        /// <param name="ct"></param>
        /// <returns></returns>
        public void ImportFrom(RenderSetupData rsetup,ClothSerializeData2 sdata2 = null)
        {
            try
            {
                ...
                if (rsetup.setupType == RenderSetupData.SetupType.Mesh)
                {
                    ...
                    isBoneCloth = false;
                    ImportMeshTypeWithColorFilter(rsetup, indices, sdata2);
            ...
            finally
            {
                ...
                [BurstCompile]
        struct Import_GetFilterRelationshipJob : IJob
        {
            public Color32 filterColor;
            [ReadOnly]
            public NativeArray<Color32> colorArray;
            [WriteOnly]
            public NativeParallelHashMap<int,int> originToFiltered;
            public void Execute()
            {
                int filteredIndex = 0;
                for (int i = 0; i < colorArray.Length; i++)
                {
                    if (colorArray[i].r == filterColor.r && colorArray[i].b == filterColor.b && colorArray[i].g == filterColor.g && colorArray[i].a == filterColor.a)
                    {
                        originToFiltered.TryAdd(i, filteredIndex);
                        filteredIndex++;
                    }
                }
            }
        ...
        struct FullFilledIndicesRef_Job : IJobParallelFor
        {
            [WriteOnly]
            public NativeParallelHashMap<int, int> originToFiltered;
            [WriteOnly]
            public NativeArray<int> filteredToOrigin;
            public void Execute(int index)
            {
                originToFiltered.Add(index, index);
                filteredToOrigin[index] = index;
            }
        }

        void PrepareFilteredReferences(ClothSerializeData2 sdata2, ref Mesh.MeshData meshData, ref NativeParallelHashMap<int,int> originToFiltered, ExSimpleNativeArray<int> filteredToOrigin)
        {
            if (sdata2.filteredToOrigin != null && sdata2.filteredToOrigin.Length > 0)
            {
                filteredToOrigin.AddRange(sdata2.filteredToOrigin.Length);
                originToFiltered = new NativeParallelHashMap<int, int>(sdata2.filteredToOrigin.Length, Allocator.Persistent);
                for (int i = 0; i < sdata2.filteredToOrigin.Length; i++)
                {
                    originToFiltered.Add(sdata2.filteredToOrigin[i], i);
                    filteredToOrigin[i] = sdata2.filteredToOrigin[i];
                }
                return;
            }

            originToFiltered = new NativeParallelHashMap<int, int>(meshData.vertexCount, Allocator.Persistent);
            // 兼容无顶点色的模型，认为索引关系一一对应
            if (!meshData.HasVertexAttribute(UnityEngine.Rendering.VertexAttribute.Color))
            {
                filteredToOrigin.AddRange(meshData.vertexCount);
                new FullFilledIndicesRef_Job()
                {
                    originToFiltered = originToFiltered,
                    filteredToOrigin = filteredToOrigin.GetNativeArray(),
                }.Run(meshData.vertexCount);
                return;
            }
            var colorArray      = new NativeArray<Color32>(meshData.vertexCount,            Allocator.TempJob);
            meshData.GetColors(colorArray);
            new Import_GetFilterRelationshipJob()
            {
                originToFiltered = originToFiltered,
                filterColor = sdata2.filterColor,
                colorArray = colorArray
            }.Run();

            sdata2.filteredToOrigin = new int[originToFiltered.Count()];
            foreach(var iter in originToFiltered)
            {
                var key = iter.Key;
                var val = iter.Value;
                sdata2.filteredToOrigin[val] = key;
            }
            filteredToOrigin.AddRange(sdata2.filteredToOrigin.Length);
            for (int i = 0; i < sdata2.filteredToOrigin.Length; i++)
            {
        ...
        void ImportMeshTypeWithColorFilter(RenderSetupData rsetup, int[] transformIndices, ClothSerializeData2 sdata2)
        {
            // root bone
            ...
            NativeParallelHashMap<int, int> originToFiltered = new NativeParallelHashMap<int, int>();
            var positionTemp      = new NativeArray<Vector3>(meshData.vertexCount, Allocator.Persistent);
            var normalTemp        = new NativeArray<Vector3>(meshData.vertexCount, Allocator.Persistent);
            var uvTemp            = new NativeArray<Vector2>(meshData.vertexCount, Allocator.Persistent);

            PrepareFilteredReferences(sdata2, ref meshData, ref originToFiltered, referenceIndices);
            int vcnt = referenceIndices.Count;
            ...
            meshData.GetVertices(positionTemp);
            
            var posJob = new Import_CopyByFilterColorJob<Vector3>() { dstArray = localPositions.GetNativeArray<Vector3>(), originToFiltered = originToFiltered, srcArray = positionTemp };
            posJob.Run(meshData.vertexCount);
            meshData.GetNormals(normalTemp);
            new Import_CopyByFilterColorJob<Vector3>() { dstArray = localNormals.GetNativeArray<Vector3>(), originToFiltered = originToFiltered, srcArray = normalTemp }.Run(meshData.vertexCount);

            if (meshData.HasVertexAttribute(UnityEngine.Rendering.VertexAttribute.Tangent))
            {
                var tangents = new NativeArray<Vector4>(meshData.vertexCount, Allocator.TempJob);
                var tangentTemp = new NativeArray<Vector4>(vcnt, Allocator.TempJob);
                meshData.GetTangents(tangents);
                new Import_CopyByFilterColorJob<Vector4>() { dstArray = tangentTemp, originToFiltered = originToFiltered, srcArray = tangents }.Run(meshData.vertexCount);
                // tangent変換(Vector4->float3)
                localTangents.CopyFromWithTypeChangeStride(tangentTemp);
                tangents.Dispose();
                tangentTemp.Dispose();
            ...
            {
                meshData.GetUVs(0, uvTemp);
                new Import_CopyByFilterColorJob<Vector2>() { dstArray = uv.GetNativeArray<Vector2>(), originToFiltered = originToFiltered, srcArray = uvTemp }.Run(meshData.vertexCount);
            ...
            if (rsetup.hasBoneWeight)
            {
                ...
                var importBoneWeightJob1 = new Import_BoneWeightJob1()
                {
                    ...
                    OriginVertexCount = meshData.vertexCount,
                    bonesPerVertexArray = rsetup.bonesPerVertexArray,
                    originToFiltered = originToFiltered,
                    startBoneWeightIndices = startBoneWeightIndices,
                };
                ...
                {
                    startBoneWeightIndices = startBoneWeightIndices,
                    referenceIndices = referenceIndices.GetNativeArray(),
            ...
            for (int i = 0; i < meshData.subMeshCount; i++)
            {
                ...
                var triangleIndices = new NativeArray<int>(submeshData.indexCount, Allocator.TempJob);
                meshData.GetIndices(triangleIndices, i);
                List<int> filteredIndicesList = new List<int>();
                for (int j = 0; j < triangleIndices.Length; j += 3)
                {
                    var v1 = triangleIndices[j];
                    var v2 = triangleIndices[j + 1];
                    var v3 = triangleIndices[j + 2];
                    if (!originToFiltered.ContainsKey(v1) || !originToFiltered.ContainsKey(v2)|| !originToFiltered.ContainsKey(v3))
                    {
                        continue;
                    }
                    filteredIndicesList.Add(originToFiltered[v1]);
                    filteredIndicesList.Add(originToFiltered[v2]);
                    filteredIndicesList.Add(originToFiltered[v3]);
                }

                // 追加
                var filteredNativeList = filteredIndicesList.ToNativeArray<int>(Allocator.TempJob);
                triangles.AddRangeTypeChange(filteredNativeList);
                filteredNativeList.Dispose();
                triangleIndices.Dispose();
            }
            originToFiltered.Dispose();
            positionTemp.Dispose();
            normalTemp.Dispose();
            uvTemp.Dispose();
        }
        
        [BurstCompile]
        struct Import_CopyByFilterColorJob<T> : IJobParallelFor where T : struct
        {
            [Unity.Collections.ReadOnly]
            public NativeArray<T> srcArray;
            [Unity.Collections.ReadOnly]
            public NativeParallelHashMap<int, int> originToFiltered;
            ...
            public void Execute(int index)
            {
                if (originToFiltered.ContainsKey(index))
                {
                    dstArray[originToFiltered[index]] = srcArray[index];
                }
            }
        }
        ...
        struct Import_BoneWeightJob1 : IJob
        {
            ...
            public void Execute()
            {
                ...
                {
                    if (originToFiltered.ContainsKey(i))
                    {
                        startBoneWeightIndices[originToFiltered[i]] = sindex;
                    }
        ...
        struct Import_BoneWeightJob2 : IJobParallelFor
        {
            ...
            public void Execute(int vindex)
            {
                ...
                int wcnt = bonesPerVertexArray[referenceIndices[vindex]];
        ...
        public void ImportFrom(RenderData renderData, ClothSerializeData2 sdata2)
        {
            try
            {
                if (renderData == null)
                ...
                ImportFrom(renderData.setupData,sdata2);
            }
